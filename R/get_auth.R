...
#' @description get_auth authenticates the R app at the Google authentication server using OAUTH2 and receives the client token.
...
get_auth = function() {
  ...
  if(exists('credentials')){
    ...
    RCurl::getURL(url,
                  ...
                  ssl.verifypeer = TRUE)
    browseURL(url)
    # Manual next-step: input code-parameter to c.token variable and run load_token()
