...
#' @description get_auth authenticates the R app at the
#' Google authentication server using OAUTH2 and receives the client token.
#' Usually you need not to run get_auth() explicitly since the whole authentication process is managed by \code{\link{do_auth}}.
...
get_credentials <- function() {
  ...
  # the developer token from the Adwords MCC and
  ...
  if (!exists("credentials")) {
    cat("Authentication process needs your Client ID
        from the Adwords API project for native apps.")
    ...
    if (c.id == "") {
      ...
      cat("Authentication process needs your Client
          secret from the Adwords API project.")
      ...
      cat("Authentication process needs your
          Developer Token from the Adwords MCC.")
  ...
  if (exists("credentials")) {
    ...
    # cert <- system.file("CurlSSL", "ca-bundle.crt",
    # package = "RCurl")#SSL Certificate Fix for Windows
    RCurl::getURL(url,
