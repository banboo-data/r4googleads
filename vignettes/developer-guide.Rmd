...
and make sure fine with publishing under the license we use. 

#### Fork & Feature Branch Based Collaboration

Even as the core team of {r4googleads} we develop new features be creating a new git branch first. For example when working on this documentation, we create a documentation specific branch that will be merged into our main branch after review once we're done working on it. This allows other contributors to work on other features with as little interference as possible. Also, in case a specific feature or developer has a longer development cycle than others, they can simply merge the main branch that might have new features in the meantime, to their feature branch from time to time. 
